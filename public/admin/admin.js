"use strict";
const i18n = require("../include/helpers").i18n;
import "./admin.css";
(function () {
	let admin = null;
	...
	const sendAlert = function (username) {
		return $("<input>")
			.attr("type", "text")
			.attr("placeholder", __("Send alert..."))
				...
				self.elements.prompt
					...
					.append(
						$("<p>")
							...
							.text(s),
						$("<select>")
							.append($("<option>").text(__("Rule #1: Hateful/derogatory speech or symbols")), $("<option>").text(__("Rule #2: Nudity, genitalia, or non-PG-13 content")), $("<option>").text(__("Rule #3: Multi-account")), $("<option>").text(__("Rule #4: Botting")), $("<option>").attr("value", "other").text(__("Other (specify below)")))
							.css({
								width: "100%",
								"margin-bottom": "1em",
							}),
						$("<textarea>")
							.attr("placeholder", __("Additional information (if applicable)"))
							.css({
								width: "100%",
								height: "5em",
							})
	...
	const style = (function () {
		var self = {
			elements: {
				sheet: $("<link>", { rel: "stylesheet", href: "/admin.css" }),
			},
			init: function () {
		...
		const self = {
			...
			callback: function (data) {
				...
				const items = [
					...
					[__("Logins"), data.logins ? data.logins.map(({ serviceID, serviceUserID }) => `${serviceID}:${serviceUserID}`).join(", ") : null],
						...
						$("<div>")
							...
							.append(
								$.map(items, function ([name, value]) {
									...
									return $("<div>").append($("<b>").text(name + ": "), typeof value === "string" ? $("<span>").text(value) : value);
								}),
								...
								$("<div>").append(
									...
									admin.user.hasPermission("user.permaban")
										? genButton(__("Permaban")).click(function () {
								...
								$("<div>").append(
									...
									admin.user.hasPermission("user.shadowban")
										? genButton(__("Shadowban")).click(function () {
								...
								admin.user.hasPermission("user.namechange.flag")
									? crel(
											"div",
											crel(
												"button",
											...
											admin.user.hasPermission("user.namechange.force")
												? crel(
														"button",
														{
															class: "text-button",
			...
			popUnban: (username) => {
				...
				admin.modal.show(admin.modal.buildDom(crel("h2", { class: "modal-title" }, __("Unban")), unbanWrapper));
			},
		};
		return {
			init: self.init,
					...
					.append(
						...
						$("<div>").append(
							...
							$.map(
								...
								function (cbox) {
									return $("<label>").append($("<input>").attr("type", "checkbox").prop("checked", !!cbox.checkState).prop("disabled", !!cbox.disabled).change(cbox.onChange), $('<span class="label-text">').text(cbox.text));
								}
							),
