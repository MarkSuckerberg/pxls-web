...
(async () => {
	...
	if (location.search) {
		const _search = location.search
			.substr(location.search.startsWith("?") ? 1 : 0)
			.split("&")
			.map((x) => x.split("="));
...
export function initInPageTabNavigation(page) {
	...
	return new Promise(function (resolve) {
		...
		popModal(
			crel(
				...
				crel("div", { class: "modal-dialog", role: "document" }, crel("div", { class: "modal-content" }, crel("div", { class: "modal-header" }, crel("h5", { class: "modal-title" }, "Confirmation")), crel("div", { class: "modal-body" }, crel("div", crel("p", text))), crel("div", { class: "modal-footer" }, noButton, yesButton)))
			)
		);
	});
}
...
function handleCollapseIconToggle(colTarget, elIcon) {
	...
	$(colTarget)
		.collapse("toggle")
		.one(wasCollapsed ? "shown.bs.collapse" : "hidden.bs.collapse", function () {
			// bind to ensure we show the correct icon once collapse fires
...
function popModal(modalDom, focusInput) {
	$(modalDom)
		.on("shown.bs.modal", () => focusInput && focusInput.focus && focusInput.focus())
		.on("hidden.bs.modal", () => $(modalDom).modal("dispose").remove())
		.modal({ keyboard: false, backdrop: "static", focus: true, show: true });
}
