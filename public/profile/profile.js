...
import { initInPageTabNavigation } from "./root.js";
				...
				if (Array.isArray(res.details)) {
					if (res.details.length > 0) {
						...
						crel(
							searchTarget,
							res.details.map((x) => {
								const btnCollapseToggle = crel("button", { class: "btn btn-sm mr-2 btn-info", "data-action": "collapse" }, crel("i", { class: "fas fa-chevron-down" }));
								const btnJoin = crel("button", { class: "btn btn-sm btn-success ml-1" }, crel("i", { class: "fas fa-user-plus mr-1" }), x.userJoined === true ? "Already Joined" : "Join");
								btnJoin.disabled = x.userJoined === true;

								const card = crel(
									"div",
									{ class: "card collapseable mb-2" },
									crel(
										"div",
										{ class: "card-header d-flex align-items-center" },
										crel("div", { class: "flex-shrink-1" }, btnCollapseToggle),
										crel("div", { class: "d-flex flex-grow-1 justify-content-between align-items-center" }, crel("span", { class: "faction-name" }, `(ID: ${x.id}) [${x.tag}] ${x.name}, from ${x.owner}`), crel("div", { class: "d-inline-block" }, btnJoin))
									),
									crel(
										"div",
										{ class: "card-body p-0 body-collapse-target collapse" },
										crel(
											"div",
											{ class: "p-3" },
											crel(
												...
												crel(
													"tbody",
													crel("tr", crel("th", { class: "text-right pr-3" }, "ID"), crel("td", { class: "text-left" }, x.id)),
													crel("tr", crel("th", { class: "text-right pr-3" }, "Name"), crel("td", { class: "text-left" }, x.name)),
													crel("tr", crel("th", { class: "text-right pr-3" }, "Tag"), crel("td", { class: "text-left" }, x.tag)),
													crel("tr", crel("th", { class: "text-right pr-3" }, "Owner"), crel("td", { class: "text-left" }, crel("a", { href: `/profile/${x.owner}`, target: "_blank" }, x.owner))),
													crel("tr", crel("th", { class: "text-right pr-3" }, "Member Count"), crel("td", { class: "text-left" }, x.memberCount || -1)),
													crel("tr", crel("th", { class: "text-right pr-3" }, "Created Date"), crel("td", { class: "text-left" }, new Date(x.creation_ms).toString())),
													crel("tr", crel("th", { class: "text-right pr-3" }, "Created Canvas"), crel("td", { class: "text-left" }, x.canvasCode))
												)
											)
										)
									)
...
function popFactionModal(isEdit = false, data = {}) {
	...
	const frmMain = crel(
		"form",
		{ action: "#", method: "POST" }, // abusing forms for required input checks
		crel("h5", { class: "mx-0 mt-0 mb-3" }, "Faction Details"),
		crel(
			"div",
			{ class: "ml-3" },
			crel("div", { class: "input-group my-0" }, crel("div", { class: "input-group-prepend" }, crel("span", { class: "input-group-text" }, "Faction Name:")), txtFactionName),
			...
			crel("div", { class: "input-group mt-3 mb-0" }, crel("div", { class: "input-group-prepend" }, crel("span", { class: "input-group-text" }, "Faction Tag:")), txtFactionTag),
			crel("small", { class: "text-muted" }, `Your tag is displayed next to your name in chat. Maximum of ${_tagMax} characters.`),
			crel("div", { class: "input-group mt-3 mb-0" }, crel("div", { class: "input-group-prepend" }, crel("span", { class: "input-group-text" }, "Faction Color:")), txtFactionColor),
			crel("small", { class: "text-muted" }, "Your faction color changes your tag color in chat."),
			crel("div", { class: "mt-3 text-right" }, btnCancel, btnSave)
		)
	);
	...
	popModal(
		crel(
			"div",
			{ class: "modal", "data-backdrop": "static", "data-keyboard": "false" }, // tabindex is interfering with the color picker's inputs. see https://github.com/bgrins/spectrum/issues/161#issuecomment-118562439
			crel("div", { class: "modal-dialog" }, crel("div", { class: "modal-content" }, crel("div", { class: "modal-header" }, crel("h5", { class: "modal-title" }, `${isEdit ? "Edit" : "Create"} Faction`)), crel("div", { class: "modal-body" }, frmMain)))
		),
		txtFactionName
	);
}
