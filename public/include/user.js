	...
	const self = {
		...
		webinit: function (data) {
			if (data.legal.termsUrl) {
				self.elements.legal.html(__('By logging in or registering, you agree to the <a href="{0}" target="_blank">terms of use</a> and <a href="{1}" target="_blank">privacy policy</a>.').replace("{0}", data.legal.termsUrl).replace("{1}", data.legal.privacyUrl));
			}
		...
		init: function (instaban) {
			...
			socket.on("userinfo", function (data) {
				...
				self.elements.userInfo.find("span#username").html(
					crel(
						"a",
						{
				...
				if (isBanned) {
					...
					crel(banelem, crel("p", __("If you think this was an error, please contact us using one of the links in the info tab.")), crel("p", __("Ban reason:")), crel("p", data.banReason));
					modal.show(modal.buildDom(crel("h2", __("Banned")), banelem), { escapeClose: false, clickClose: false });
					if (window.deInitAdmin) {
						window.deInitAdmin();
					}
		...
		_handleRenameClick: function (event) {
			const renamePopup = crel(
				"form",
				{ onsubmit: self._handleRenameSubmit },
				...
				crel("div", { style: "text-align: right" }, crel("button", { class: "text-button", onclick: () => modal.closeAll() }, __("Not now")), crel("button", { class: "rename-submit text-button", type: "submit" }, __("Change")))
			);
			modal.show(modal.buildDom(crel("h2", { class: "modal-title" }, __("Rename Requested")), renamePopup));
		},
		showRenameRequest: () => {
			self.elements.userMessage
				.empty()
				.show()
				.append(
					crel(
						"span",
						__("You must change your username.") + " ",
						crel(
							"span",
							{
