	...
	const self = {
			...
			modal.show(
				modal.buildDom(
					...
					crel(
						...
						crel("div", { class: "buttons" }, crel("button", { class: "text-button", onclick: () => modal.closeAll() }, __("Cancel")), reportButton)
					)
				)
			);
		},
		...
		create: function (data) {
			...
			self._makeShell(data)
				...
				.append(() => {
					const hooks = data.bg ? self.hooks.filter((hook) => hook.backgroundCompatible) : self.hooks;
					...
					return $.map(hooks, (hook) => {
						...
						const value = typeof get === "object" ? (get instanceof Node ? $(get) : get) : $("<span>").text(get);

						const _retVal = $('<div data-sensitive="' + hook.sensitive + '">')
							.append($("<b>").text(hook.name + ": "), value.css(hook.css))
		...
		_makeShell: function (data) {
			return self.elements.lookup.empty().append(
				...
				!data.bg && user.isLoggedIn()
					? $("<button>")
							.css("float", "left")
							.addClass("dangerous-button text-button")
							.text(__("Report"))
							.click(function () {
				...
				template.getOptions().use
					? $("<button>")
							.css("float", "right")
							.addClass("text-button")
							.text(__("Move Template Here"))
							.click(function () {
		...
		runLookup(clientX, clientY) {
			...
			$.get("/lookup", pos, function (data) {
				...
				self._makeShell({ x: pos.x, y: pos.y })
					.find(".content")
					.first()
					.append($("<p>").css("color", "#c00").text(__("An error occurred, either you aren't logged in or you may be attempting to look up users too fast. Please try again in 60 seconds")));
				self.elements.lookup.fadeIn(200);
			});
		},
		webinit: function () {
			...
			self.registerHook(
				{
					...
					get: (data) =>
						$("<a>")
							.text("(" + data.x + ", " + data.y + ")")
							.attr("href", coords.getLinkToCoords(data.x, data.y)),
