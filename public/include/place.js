...
const { analytics, hexToRGB } = require("./helpers");
...
module.exports.place = (function () {
	const self = {
		...
		switch: function (newColorIdx) {
			...
			[self.elements.cursor, self.elements.reticule].forEach((el) => el.css("background-color", isOnPalette ? `#${self.palette[newColorIdx].value}` : isTransparent ? "var(--general-background)" : null));
			if (newColorIdx !== -1) {
		...
		place: function (x, y, color = null) {
			if ((!timer.cooledDown() && !uiHelper.getAvailable()) || self.color === -1) {
		...
		setPalette: function (palette) {
				...
				.append(
					$.map(self.palette, function (color, idx) {
						const button = $("<button>")
							...
							.append($("<div>").addClass("palette-color").css("background-color", `#${color.value}`).append($("<span>").addClass("palette-number").text(idx)))
							.click(function () {
								self.switch(idx);
							});
			...
			self.elements.palette.prepend(
				$("<button>")
					...
					.append(crel("i", { class: "fas fa-times" }))
					.click(function () {
						self.switch(-1);
					})
			);
		...
		init: function () {
			...
			$(window)
				.on("pointermove mousemove touchstart touchmove", function (evt) {
					let x = 0;
					let y = 0;
