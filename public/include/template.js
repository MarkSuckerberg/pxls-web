...
module.exports.template = (function () {
	...
	const self = {
		...
		updateSettings: function () {
			...
			self.elements.titleInput.prop("disabled", !self.options.use).val(self.options.title ? self.options.title : "");
			...
			self.elements.coordsXInput.prop("disabled", !self.options.use).val(self.options.x);
			self.elements.coordsYInput.prop("disabled", !self.options.use).val(self.options.y);
		...
		isDirty: function () {
			return parseFloat(self.elements.visibles.css("width")) !== self.getDisplayWidth() || parseFloat(self.elements.visibles.attr("width")) !== self.getInternalWidth() || parseFloat(self.elements.visibles.attr("height")) !== self.getInternalHeight();
		},
		...
		init: function () {
			...
			self.elements.visibles.data("dragging", false).on("mousedown pointerdown", function (evt) {
		...
		webinit: function (data) {
			self.initGl(
				self.elements.template[0].getContext("webgl", {
		...
		loadStyle: function (redraw = true) {
			...
			if (self.gl.context !== null && style.naturalWidth !== 0 && style.naturalHeight !== 0) {
				...
				self.gl.context.texImage2D(self.gl.context.TEXTURE_2D, 0, self.gl.context.ALPHA, self.gl.context.ALPHA, self.gl.context.UNSIGNED_BYTE, style);
				if (redraw) {
		...
		initGl: function (context, palette) {
			...
			self.gl.context.framebufferTexture2D(self.gl.context.FRAMEBUFFER, self.gl.context.COLOR_ATTACHMENT0, self.gl.context.TEXTURE_2D, self.gl.textures.downscaled, 0);
			...
			self.gl.context.bufferData(self.gl.context.ARRAY_BUFFER, new Float32Array([-1, -1, -1, 1, 1, -1, 1, 1]), self.gl.context.STATIC_DRAW);
      ...
      `;
			...
			self.gl.programs.stylize = self.createGlProgram(
      ...
      `
		...
		downscaleTemplate: function () {
			...
			self.gl.context.texImage2D(self.gl.context.TEXTURE_2D, 0, self.gl.context.RGBA, width, height, 0, self.gl.context.RGBA, self.gl.context.UNSIGNED_BYTE, null);
			...
			self.gl.context.uniform2f(self.gl.context.getUniformLocation(program, "u_SampleSize"), Math.max(1, self.getDownscaleWidthRatio()), Math.max(1, self.getDownscaleHeightRatio()));
			self.gl.context.uniform2f(self.gl.context.getUniformLocation(program, "u_TexelSize"), 1 / self.getDisplayWidth(), 1 / self.getDisplayHeight());

			self.gl.context.bindTexture(self.gl.context.TEXTURE_2D, self.gl.textures.source);

			const texture = self.elements.sourceImage[0];

			self.gl.context.texImage2D(self.gl.context.TEXTURE_2D, 0, self.gl.context.RGBA, self.gl.context.RGBA, self.gl.context.UNSIGNED_BYTE, texture);
		...
		stylizeTemplate: function () {
			...
			self.gl.context.uniform2f(self.gl.context.getUniformLocation(self.gl.programs.stylize, "u_TexelSize"), 1 / self.getDisplayWidth(), 1 / self.getDisplayHeight());
