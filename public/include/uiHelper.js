...
const uiHelper = (function () {
	const self = {
		...
		init: function () {
			...
			socket.on("alert", (data) => {
				modal.show(modal.buildDom(crel("h2", { class: "modal-title" }, "Alert"), crel("p", { style: "padding: 0; margin: 0;" }, chat.processMessage(data.message)), crel("span", `Sent from ${data.sender || "$Unknown"}`)), { closeExisting: false });
			});
			...
			$("#lock-button i").toggleClass("fa-lock", settings.board.lock.enable.get()).toggleClass("fa-unlock", !settings.board.lock.enable.get());
			...
			settings.board.lock.enable.listen(function (value) {
				$("#lock-button i").toggleClass("fa-lock", value).toggleClass("fa-unlock", !value);
			});
		...
		_initThemes: function () {
			for (let i = 0; i < self.themes.length; i++) {
				...
				self.elements.themeSelect.append(
					$("<option>", {
						value: i,
		...
		_initAudio: function () {
			...
			settings.audio.alert.src.listen(function (url) {
				// change should only fire on blur so we normally won't be calling updateAudio for each keystroke. just in case though, we'll lazy update.
				if (self._alertUpdateTimer !== false) clearTimeout(self._alertUpdateTimer);
				self._alertUpdateTimer = setTimeout(
					function (url) {
		...
		_initMultiTabDetection() {
			...
			if (serviceWorkerHelper.hasSupport) {
				...
				serviceWorkerHelper.readyPromise
					.then(async () => {
		...
		handleDiscordNameSet() {
			...
			$.post({
				...
				error: function (data) {
					const err = data.responseJSON && data.responseJSON.details ? data.responseJSON.details : data.statusText;
					if (data.status === 200) {
						// seems to be caused when response body isn't json? just show whatever we can and trust server sent good enough details.
						modal.showText(err);
					} else {
		...
		adjustColorBrightness(level) {
			$(["#board-container", "#cursor", "#reticule", "#palette .palette-color"].join(", ")).css("filter", level != null ? `brightness(${level})` : "");
		},
		getAvailable() {
		...
		styleElemWithChatNameColor: (elem, colorIdx, layer = "bg") => {
			elem.classList.remove(
				...self.specialChatColorClasses.reduce((acc, val) => {
	...
	return {
		...
		tabHasFocus: () => {
			return serviceWorkerHelper.hasSupport ? self._workerIsTabFocused : ls.get("tabs.has-focus") === self.tabId;
		},
