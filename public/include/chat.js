...
const chat = (function () {
	const self = {
		...
		init: async () => {
			...
			self.markdownProcessor = uiHelper.makeMarkdownProcessor().use(function () {
				...
				this.Compiler.prototype.visitors.coordinate = (node, next) => self._makeCoordinatesElement(node.url, node.x, node.y, node.scale);
			});
				...
				if (e.target && Array.isArray(e.history) && Array.isArray(e.chatbans)) {
					...
					const dom = crel(
						...
						crel(
							...
							e.history.length > 0
								? [
										// array children are injected as fragments
										crel("h3", { style: "text-align: center" }, `Last ${e.history.length} messages`),
										crel("hr"),
										crel(
											"ul",
											{ class: "chat-history chat-body" },
											e.history.map((message) =>
												crel(
													"li",
													{ class: `chat-line ${message.purged ? "purged" : ""}`.trimRight() },
								...
								: [crel("h3", { style: "text-align: center" }, "Last Messages"), crel("hr"), crel("p", "No message history")]
						),
						crel(
							"div",
							{ class: "side chat-lookup-side" },
								...
								e.chatbans.map((chatban) => {
									return crel(
										...
										crel(
											"article",
											{ class: "chatban" },
											crel("header", crel("h4", `${chatban.initiator_name} ${chatban.type === "UNBAN" ? "un" : ""}banned ${e.target.username}${chatban.type !== "PERMA" ? "" : ""}`)),
											crel(
												"div",
												crel(
													"table",
													crel(
														"tbody",
														crel("tr", crel("th", "Reason:"), crel("td", chatban.reason || "$No reason provided$")),
														crel("tr", crel("th", "When:"), crel("td", moment(chatban.when * 1e3).format(longFormat))),
														chatban.type !== "UNBAN"
															? [
																	crel("tr", crel("th", "Length:"), crel("td", chatban.type.toUpperCase().trim() === "PERMA" ? "Permanent" : `${chatban.expiry - chatban.when}s${chatban.expiry - chatban.when >= 60 ? ` (${moment.duration(chatban.expiry - chatban.when, "seconds").humanize()})` : ""}`)),
																	chatban.type.toUpperCase().trim() === "PERMA" ? null : crel("tr", crel("th", "Expiry:"), crel("td", moment(chatban.expiry * 1e3).format(longFormat))),
																	crel("tr", crel("th", "Purged:"), crel("td", String(chatban.purged))),
															  ]
															: null
													)
												)
					...
					modal.show(modal.buildDom(crel("h2", { class: "modal-title" }, "Chat Lookup"), dom));
				}
			});
			...
			function handleIgnoreChange(who, isIgnored) {
				...
				Array.from(document.querySelectorAll(`.reply-preview[data-author="${who}"]`)).forEach((x) => {
					...
					if (!isIgnored) return;
					parent.prepend(crel("div", { class: "reply-preview reply-preview-nojump", "data-ignore-text": "1" }, crel("i", { class: "fas fa-reply fa-flip-horizontal" }), "Message was ignored"));
				});
			...
			self.elements.pings_button[0].addEventListener("click", function () {
				...
				const pingsList = crel(
					"ul",
					{ class: "pings-list" },
					self.pingsList.map((packet) => {
						console.info(packet);
						const _processed = crel("span", self.processMessage(packet.message_raw));
						return crel(
							"li",
							{ title: _processed.textContent },
							crel("i", {
			...
			selSettingChatLinksInternalBehavior.append(Object.values(self.TEMPLATE_ACTIONS).map((action) => crel("option", { value: action.id }, action.pretty)));
		...
		async webinit(data) {
			...
			if (data.chatEnabled) {
				...
				self.customEmoji = [...data.customEmoji.map(({ name, emoji }) => ({ name, emoji: `./emoji/${emoji}` })), ...emotes7TV];
		...
		initTypeahead() {
			// init DBs
			const dbEmojis = new TH.Database(
				"emoji",
				{},
				false,
				false,
				(x) => (twemoji.test(x.value) ? x.value : ":" + x.key + ":"),
				(x) => (twemoji.test(x.value) ? `${twemoji.parse(x.value)} :${x.key}:` : `${'<img class="emoji emoji--custom" draggable="false" alt="' + x.key + '" src="' + x.value + '"/>'} :${x.key}:`)
			);
			...
			// attach events
			self.elements.typeahead[0].querySelectorAll('[data-dismiss="typeahead"]').forEach((x) =>
			...
			self.elements.input[0].addEventListener("keyup", function (event) {
				...
				if (self.elements.input[0].value.length !== self.typeahead.lastLength) {
					scan();
				}
			});
		...
		selectNextTypeaheadEntry(direction) {
			...
			if (direction < 0 && nextIndex < 0) {
				// if we're walking backwards, we need to check for underflow.
				nextIndex = children.length - 1;
			} else if (direction > 0 && nextIndex >= children.length) {
				// if we're walking forwards, we need to check for overflow.
				nextIndex = 0;
			}
		...
		reloadIgnores: () => {
			self.ignored = (ls.get("chat.ignored") || "").split(",");
		},
		...
		_handlePingJumpClick: function () {
			// must be es5 for expected behavior. don't upgrade syntax, this is attached as an onclick and we need `this` to be bound by dom bubbles.
		...
		addServerAction: (msg) => {
			const when = moment();
			const toAppend = crel("li", { class: "chat-line server-action" }, crel("span", { title: when.format("MMM Do YYYY, hh:mm:ss A") }, when.format(settings.chat.timestamps["24h"].get() === true ? "HH:mm" : "hh:mm A")), document.createTextNode(" - "), crel("span", { class: "content" }, msg));
		...
		_populateUsernameColor: () => {
			...
			self.elements.username_color_select.empty().append(
				...
				place.palette.map(({ name, value: hex }, i) =>
					crel(
						"option",
						{
							value: i,
		...
		_updateFaction: (faction) => {
			...
			self.elements.body.find(`.chat-line[data-faction="${faction.id}"], .reply-preview[data-faction="${faction.id}"]`).each(function () {
				...
				$(this)
					.children("span")
					.find(".faction-tag")
					.attr("data-tag", faction.tag)
					.attr("title", `${faction.name} (ID: ${faction.id})`)
					.css("color", colorHex)
					.html(`[${twemoji.parse(faction.tag)}]`);
			});
		},
		_clearFaction: (fid) => {
			if (fid == null) return;
		...
		_process: (packet, isHistory = false) => {
			...
			const messageDisplay = crel("span", {}, userDisplay, document.createTextNode(": "), contentSpan, document.createTextNode(" "));
			...
			if (hasPing && !isIgnored && !packet.purge) {
				...
				const canPlayPingAudio = !isHistory && settings.audio.enable.get() && pingAudioState !== "off" && Date.now() - self.lastPingAudioTimestamp > 5000;
				if ((!panels.isOpen("chat") || !document.hasFocus() || pingAudioState === "always") && uiHelper.tabHasFocus() && canPlayPingAudio) {
		...
		processMessage: (str, mentionCallback) => {
			let content = str;
			try {
				const processor = self.markdownProcessor().use(pxlsMarkdown.plugins.mention, { mentionCallback });
		...
		_generateReplyPreview: (replyingToId, replyShouldMention, hasPing) => {
			...
			if (!replyTarget[0]) {
				return {
					div: crel("div", { class: "reply-preview reply-preview-nojump" }, crel("i", { class: "fas fa-reply fa-flip-horizontal" }), "Message couldn't be found"),
					hasPing,
				};
			}
			...
			const output = [
				crel(
					"div",
					{
						...
						dataset: ["id", "tag", "faction", "author", "date"].reduce((obj, key) => {
							obj[key] = replyTarget[0].dataset[key];
							return obj;
						}, {}),
			...
			if (isIgnored) {
				output.unshift(crel("div", { class: "reply-preview reply-preview-nojump", "data-ignore-text": "1" }, crel("i", { class: "fas fa-reply fa-flip-horizontal" }), "Message was ignored"));
			}
			return {
				div: output,
				hasPing,
		...
		_makeCoordinatesElement: (raw, x, y, scale, template, title) => {
			...
			if (template != null && template.length >= 11) {
				// we have a template, should probably make that known
		...
		_handleTemplateOverwriteAction: (action, linkElem) => {
			switch (action) {
				...
				case self.TEMPLATE_ACTIONS.NEW_TAB.id: {
					if (!window.open(linkElem.dataset.raw, "_blank")) {
						// what popup blocker still blocks _blank redirects? idk but i'm sure they exist.
						modal.show(
							modal.buildDom(
					...
					modal.buildDom(
						...
						crel(
							...
							Object.values(self.TEMPLATE_ACTIONS).map((action) =>
								action.id === self.TEMPLATE_ACTIONS.ASK.id
									? null
									: crel(
											"label",
		...
		_handleActionClick: function (e) {
			...
			switch (this.dataset.action.toLowerCase().trim()) {
				case "report": {
					...
					const chatReport = crel(
						"form",
						{ class: "report chat-report", "data-chat-id": this.dataset.id },
						...
						$("<p>")
							...
							.html(i18n(__("You are reporting a chat message from {reportTarget} with the content: "), { reportTarget: reportTarget }) + reportMessage)[0],
						textArea,
						crel(
							"div",
					...
					modal.show(modal.buildDom(crel("h2", { class: "modal-title" }, __("Report User")), chatReport));
					break;
				}
				case "mention": {
					if (reportingTarget) {
				...
				case "chatban": {
					const messageTable = mode
						? crel("table", { class: "chatmod-table" }, crel("tr", crel("th", "ID: "), crel("td", this.dataset.id)), crel("tr", crel("th", "Message: "), crel("td", { title: reportingMessage }, `${reportingMessage.substr(0, 120)}${reportingMessage.length > 120 ? "..." : ""}`)), crel("tr", crel("th", "User: "), crel("td", reportingTarget)))
						: crel("table", { class: "chatmod-table" }, crel("tr", crel("th", "User: "), crel("td", reportingTarget)));
					...
					const _selBanLength = crel(
						"select",
						{ name: "selBanLength" },
						banLengths.map((lenPart) => crel("option", { value: lenPart[1] }, lenPart[0]))
					);

					const _customLenWrap = crel("div", { style: "display: block; margin-top: .5rem" });
					const _selCustomLength = crel(
						"select",
						{
					...
					const _selBanReason = crel("select", crel("option", __("Rule 3: Spam")), crel("option", __("Rule 1: Chat civility")), crel("option", __("Rule 2: Hate Speech")), crel("option", __("Rule 5: NSFW")), crel("option", __("Custom")));
					...
					const chatbanContainer = crel(
						...
						_selBanLength,
						crel(_customLenWrap, _txtCustomLength, _selCustomLength),
						crel(_reasonWrap, crel("h5", __("Reason")), _selBanReason, crel(_additionalReasonInfoWrap, _txtAdditionalReason)),
						crel(
							_purgeWrap,
							crel("h5", __("Purge Messages")),
							board.snipMode ? crel("span", { class: "text-orange extra-warning" }, crel("i", { class: "fas fa-exclamation-triangle" }), " " + __("Purging all messages is disabled during snip mode")) : [crel("label", { style: "display: inline;" }, _rbPurgeYes, __("Yes")), crel("label", { style: "display: inline;" }, _rbPurgeNo, __("No"))]
						),
						board.snipMode ? "" : crel(_silentPurgeWrap, crel("label", { style: "display: inline;" }, _cbSilentPurge, __("Silent (no purge message)"))),
						crel("div", { class: "buttons" }, _btnCancel, _btnOK)
					);
					...
					modal.show(modal.buildDom(crel("h2", { class: "modal-title" }, __("Chatban")), crel("div", { style: "padding-left: 1em" }, chatbanContainer)));
					break;
				}
				case "delete": {
					...
					const deleteWrapper = crel(
						"div",
						{ class: "chatmod-container" },
						crel(
							"table",
							crel("tr", crel("th", __("ID: ")), crel("td", this.dataset.id)),
							crel("tr", crel("th", __("User: ")), crel("td", reportingTarget)),
							crel("tr", crel("th", __("Message: ")), crel("td", { title: reportingMessage }, `${reportingMessage.substr(0, 120)}${reportingMessage.length > 120 ? "..." : ""}`)),
							crel("tr", crel("th", __("Reason: ")), crel("td", _txtReason)),
							crel("tr", crel("th", _cbSilentPurge), crel("td", crel("label", { for: "cbSilentPurge" }, __("Silent (no purge message)"))))
						),
						crel(
							"div",
							{ class: "buttons" },
					...
					modal.show(modal.buildDom(crel("h2", { class: "modal-title" }, __("Delete Message")), deleteWrapper));
					break;
				}
				case "purge": {
					...
					const messageTable = mode
						? crel("table", crel("tr", crel("th", __("ID: ")), crel("td", this.dataset.id)), crel("tr", crel("th", __("Message: ")), crel("td", { title: reportingMessage }, `${reportingMessage.substr(0, 120)}${reportingMessage.length > 120 ? "..." : ""}`)))
						: crel("table", { class: "chatmod-table" }, crel("tr", crel("th", __("User: ")), crel("td", reportingTarget)));

					const purgeWrapper = crel(
						"form",
						{ class: "chatmod-container" },
						crel("h5", __("Selected Message")),
						messageTable,
						crel("div", crel("h5", __("Purge Reason")), txtPurgeReason),
					...
					modal.show(modal.buildDom(crel("h2", { class: "modal-title" }, __("Purge User")), crel("div", { style: "padding-left: 1em" }, purgeWrapper)));
					break;
				}
				case "lookup-mod": {
				...
				case "request-rename": {
					...
					modal.show(modal.buildDom(crel("h2", { class: "modal-title" }, "Request Rename"), renameWrapper));
					break;
				}
				case "force-rename": {
					...
					modal.show(modal.buildDom(crel("h2", { class: "modal-title" }, __("Force Rename")), renameWrapper));
					break;
				}
				case "profile": {
		...
		_doScroll: (elem) => {
			try {
				// Fixes iframes scrolling their parent. For context see https://github.com/pxlsspace/Pxls/pull/192's commit messages.
		...
		updateCanvasBanState(state) {
			...
			if (!canChat) {
				if (self.elements.rate_limit_counter.text().trim().length === 0) {
					self.elements.rate_limit_counter.text(__("You cannot use chat while canvas banned."));
				}
			}
		},
	};
